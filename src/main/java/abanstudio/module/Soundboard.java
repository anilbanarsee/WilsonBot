...
//		overrideMap = new HashMap<>();
		...
		}.setOverride(true));//override
		actionMap.put("play", new Action()
		{
		...
		actionMap.put("addclip", new Action()
		{
			public void exec(String[] arg, IMessage m)
			{
				addClip(arg, m);
		...
		actionMap.put("deleteclip", new Action()
		{
			public void exec(String[] arg, IMessage m)
			{
				deleteClip(arg, m);
		...
		actionMap.put("setvolume", new Action()
		{
			public void exec(String[] arg, IMessage m)
			{
				setVolume(arg, m);
		...
		actionMap.put("banclip", new Action()
		{
			public void exec(String[] arg, IMessage m)
			{
				ban(arg, m);
		...
		actionMap.put("unbanclip", new Action()
		{
			public void exec(String[] arg, IMessage m)
			{
				unban(arg, m);
		...
		actionMap.put("vetoclip", new Action()
		{
			public void exec(String[] arg, IMessage m)
			{
				veto(arg, m);
	...
	{
		ArrayList<CommandData> commList = new ArrayList<CommandData>();
		commList.add(new CommandData("[jJ]oin", "join", "joins a channel, will skip any clips currently banned by users within the channel"));
		...
		if (arguments.length < 3) {
			...
			try {
				...
				for (int i = 2; i < arguments.length; i++) {
					...
					realTimes[i - 2] = LocalTime.of(h, m, (int) s, (int) ms);
				}
